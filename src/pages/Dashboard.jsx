...
// ... rest of your component code (the function Dashboard() { ... }) ...
...
function Dashboard() {
  ...
  useEffect(() => {
    // FIXED: Get user from 'user' key (not 'rm')
  ...
  const loadTargets = async (rmId) => {
    try {
      // Get targets from Supabase directly
  ...
  const loadDashboardData = async (rmId, range) => {
    ...
    try {
      ...
      // Get all data from Supabase directly
      ...
      // Check for errors
      if (cpResult.error) console.error('CP error:', cpResult.error);
      if (meetingResult.error) console.error('Meeting error:', meetingResult.error);
      if (salesResult.error) console.error('Sales error:', salesResult.error);
      ...
      console.log('Filtered data:', { rmCPs, rmMeetings, rmSales });
      ...
      // Calculate totals - using sale_amount field
      ...
      // Generate predictions if targets exist
      if (targets) {
        setPredictions({
